...
**Aggregate scoring across workloads.** Since performance is measured across many tasks, you need a way to summarize. AlgoPerf uses **performance profiles** (Dolan and Moré, 2002). The idea: for each task, measure how much slower you are compared to the fastest submission. Then plot the fraction of tasks you solved within a given slowdown factor $\tau$. An optimizer whose line is higher and further to the left is better, meaning it solves more problems faster. The performance profile curves can still cross, so to get a single winner, the benchmark computes a scalar score by integrating the area under each submission's performance profile curve (over a range of $\tau$ values). Higher area means you solve more workloads more quickly on aggregate.
